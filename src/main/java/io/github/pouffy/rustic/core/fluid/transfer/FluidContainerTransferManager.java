...
import io.github.pouffy.rustic.core.data.DatapackHelpers;
...
public class FluidContainerTransferManager extends RegistryAccessJsonReloadListener {
    ...
    protected FluidContainerTransferManager() {
        super(GSON, "rustic/fluid_transfer");
    }
    ...
    public void parse(Map<ResourceLocation, JsonElement> jsonMap, RegistryAccess registryAccess) {
        ...
        for (var e : map.entrySet()) {
            ...
            }, () -> Rustic.LOGGER.debug("Skipping loading fluid transfer {} as its conditions were not met", e.getKey()));
        }

        TRANSFERS.clear();
        TRANSFERS.putAll(transfers);
        this.containerItems = null;
        Rustic.LOGGER.info("Loaded {} fluid transfers in {} ms", TRANSFERS.size(), (System.nanoTime() - time) / 1000000f);
    }
